...
import { orderByFileSize } from "@/lib/validators/file";

export async function POST(request: NextRequest, response: NextResponse) {
  try {
    const temp = await request.json();
    const input = await orderByFileSize.parse(temp);
    if (
    ...
    if (input.ascending) finalResult.sort((a, b) => a.Size - b.Size);
    if (input.descending) finalResult.sort((a, b) => b.Size - a.Size);
